...
namespace Menu {
	...
	std::map<std::string, std::vector<std::string>> categories = {
		{"Settings", {"Show Keybinds", "Allow Activating Keybinds while Chatting", "Always Show Menu on Startup", "Panic (Disable VMenu)",
					  ...
					  "Replace Streamer Mode Lobby Code", "RGB Lobby Code", "Unlock Cosmetics", "Safe Mode", "Allow other VMenu users to see you're using VMenu",
	...
	void Render() {
		try {
			if (!init)
				Menu::Init();
			std::string modText = std::format("VMenu {}", State.SickoVersion);
			ImGui::Begin("VMenu", &State.ShowMenu, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollWithMouse);
			...
			ImGui::BeginChild("###VMenu", ImVec2(90 * State.dpiScale, 0), true, ImGuiWindowFlags_NoBackground);
			// Search field
