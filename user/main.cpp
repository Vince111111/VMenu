...
bool GameVersionCheck() {
    ...
    /*if (!IsWindows10OrGreater()) {
        ...
        MessageBox(NULL, L"This version of Windows is not supported!", L"YourNewMenuName", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
        return false;
    }*/

    if (!std::filesystem::exists(gameAssembly)) {
        Log.Error("GameAssembly.dll was not found");
        MessageBox(NULL, L"Unable to locate GameAssembly.dll", L"YourNewMenuName", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
        return false;
    }
...
#define GAME_STATIC_POINTER(f,c,m) \
    ...
    { \
        if (!cctor_finished(c##__TypeInfo->_0.klass)) { \
            if (MessageBox(NULL, \
                L"YourNewMenuName does not support Among Us versions past v16.0.5 as of now!\n\nMake sure you downgrade your Among Us instance to v16.0.0 / v16.0.2 and use YourNewMenuName. If you're wondering about playing with players on the latest version, your game's version will automatically be spoofed so you can play with them!\n\nClick OK to exit the game. Your browser will then open a downgrading guide.", \
                L"YourNewMenuName", MB_ICONINFORMATION)) { \
...
void Run(LPVOID lpParam) {
    ...
    {
        std::ostringstream ss;
        ss << "\n\tYourNewMenuName - " << __DATE__ << " - " << __TIME__ << std::endl; // Log YourNewMenuName info
        ...
        LOG_INFO(ss.str());
#if _DEBUG
        SetConsoleTitleA(std::format("Debug Console - YourNewMenuName {} (Among Us v{})", State.SickoVersion, getGameVersion()).c_str());
#endif
    }
#if _DEBUG
